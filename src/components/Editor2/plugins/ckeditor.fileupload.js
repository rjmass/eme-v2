import { EditorOverrides } from './ckeditor.overrides';

class FtFileUpload {

  static load(angularInjectables = {}) {

    var counter = 0;
    var postUrl = `${angularInjectables.config.baseUrl}/eme-images`;
    ...
    function getEditorCss(config) {
      var imgUpload = 'span.SimpleUploadsTmpWrapper>span { top: 50%; margin-top: -0.5em; width: 100%; text-align: center; color: #fff; ' + 'text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5); font-size: 50px; font-family: Calibri,Arial,Sans-serif; pointer-events: none; ' + 'position: absolute; display: inline-block;}';
    ...
    function pickAndUploadFile(editor, forImage, caller, callback) {
      ...
      if (typeof FormData === 'undefined') {
        ...
        var form = '<form method=\'post\' enctype=\'multipart/form-data\' action=\'' + getUploadUrl(editor, fnNumber, forImage) + '\'>' + '<input type=\'file\' name=\'upload\' id=\'upload\'></form>';
        var src = 'document.open(); document.write("' + form + '");document.close();' + 'window.parent.CKEDITOR.tools.callFunction(' + fnInitPicker + ');';
        iframe.src = 'javascript:void(function(){' + encodeURIComponent(src) + '}())'; //jshint ignore:line
        // Detect when the file upload ends to check for errors
        iframe.onreadystatechange = function() {
          if (iframe.readyState === 'complete') {
            window.setTimeout(function() {
              if (IEUpload_fileName) {
                console.log('The file upload has failed');
    ...
    function setUrl(fileUrl, msg) {
      ...
      editor.fire('simpleuploads.endUpload', {
        name: IEUpload_fileName,
        ok: !!fileUrl
      });
      if (IEUpload_callback) {
        IEUpload_callback.upload(fileUrl, msg, {
          context: IEUpload_caller
        });
        IEUpload_fileName = null;
      ...
      if (fileUrl) {
        ...
        if (filePickerForceLink) {
          filePickerEditor.fire('simpleuploads.finishedUpload', {
            name: IEUpload_fileName,
            element: element
          });
        }
    ...
    CKEDITOR.plugins.add('simpleuploads', {
      ...
      onLoad: function() {
        ...
        content += [
          '.cke_throbber {margin: 0 auto; width: 100px;}',
      ...
      init: function(editor) {
        ...
        editor.on('simpleuploads.startUpload', function(ev) {
          ...
          if (editor.config.simpleuploads_maxFileSize && file && file.size && file.size > editor.config.simpleuploads_maxFileSize) {
            ev.cancel();
          }
        ...
        editor.on('paste', function(e) {
          ...
          if (CKEDITOR.env.webkit && html.indexOf('webkit-fake-url') > 0) {
            console.log('Sorry, the images pasted with Safari aren\'t usable');
        ...
        editor.ui.addButton('addFile', {
          label: 'add file',
          command: 'addFile',
          icon: this.path + 'icons/addfile.png',
          // %REMOVE_LINE_CORE%
          toolbar: 'insert',
        ...
        editor.on('contentDom', function() {
          ...
          root.on('paste', pasteListener, null, {
            editor: editor
          });
    ...
    function createPreview(data, editor, forceLink) {
      ...
      if (!forceLink && isImage && showImageProgress) {
        ...
        var html = '<span class=\'uploadName\'>' + data.name + '</span>' + ' <span class=\'uploadRect\'><span id=\'rect' + data.id + '\'></span></span>' + ' <span id=\'text' + data.id + '\' class=\'uploadText\'> </span><span class=\'uploadCancel\'>x</span>';
    ...
    function errorListener(e) {
      console.log('Failed to load the image with the provided URL: \'' + e.sender.data('cke-saved-src') + '\'');
    ...
    function checkLoadedImage(img, editor, el, name) {
      if (img.$.naturalWidth === 0) {
        ...
        window.setTimeout(function() {
          checkLoadedImage(img, editor, el, name);
        }, 50);
        return;
      }
      img.replace(el);
    ...
    function createXHRupload(editor, forceLink, data) {
      ...
      var upload = function(file) {
        ...
        var req = angularInjectables.$http.post(postUrl, formData, { headers: { 'Content-Type': undefined }});
          req.then(function (response) {
            var result = response.data;
            // proxy here, uploaded image
            var fileUrl = EditorOverrides.proxifyUrl(result.url),
              id = data.id,
          ...
          angularInjectables.Notification.success('Image uploaded');

          // if uploading from dialog, then just
          // update image url in dialog and exit
          if (data.callback) {
            data.callback.upload(fileUrl, msg, data);
            return;
          }

          // if not uploading from dialog
          // than carry on
          if (fileUrl) {
            var attribute = 'src';
            var naturalWidth = EditorOverrides.IMG_VISIBLE_WIDTH;
            ...
            if (el.$.nodeName.toLowerCase() === 'span') {
              ...
              img.setAttribute('width', Math.min(naturalWidth, EditorOverrides.IMG_VISIBLE_WIDTH));
              img.setAttribute('style', 'vertical-align: middle; margin: 0 4px;');
              ...
              // make editor aware of the change
        ...
        .catch(function() {
          ...
          // Correct undo image
          angularInjectables.Notification.error('Image uploaded');
          editor.fire('updateSnapshot');
        });
      };
      upload(data.file);
    ...
    function createSimpleUpload(editor, dialogName, definition, element) {
      ...
      var callback = {
        ...
        start: function(data) {
          ...
          if (data.xhr) {
            var html = '<span class=\'uploadName\'>' + data.name + '</span>' + ' <span class=\'uploadRect\'><span id=\'rect' + data.id + '\'></span></span>' + ' <span id=\'text' + data.id + '\' class=\'uploadText\'> </span><a>x</a>';
      ...
      if (element.filebrowser.action === 'QuickUpload') {
        ...
        if (!editor.config.simpleuploads_respectDialogUploads) {
          element.label = dialogName.substr(0, 5) === 'image' ? 'Add Image' : 'Add File';
