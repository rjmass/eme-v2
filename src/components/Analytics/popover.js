...
Popover.prototype = {
  ...
  init : function() {
    ...
    if (!(this.isIE && this.isIE < 9) ) { // dismiss on window resize
      ...
      this.listeners.resize = this.close;
    }
  },
...
export function addPopovers() {
  ...
  for (i;i<ppl;i++){
    ...
    createdPopovers.push(new Popover(item,options));
  }
  return createdPopovers;
}

export function removePopoverListeners(popovers) {
  for (const popover of popovers) {
    Object.keys(popover.listeners).forEach(event => {
      window.removeEventListener(event, popover.listeners[event]);
    });
  }
}
